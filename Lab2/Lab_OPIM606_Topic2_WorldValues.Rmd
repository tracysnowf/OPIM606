...
opts_knit$set(root.dir = "C:/Users/Xue Feng/Desktop/OPIM606/OPIM606_Lab2")
```
...
# create a hash to store country info. key: country code, value: country name
countryInfo <- hash()
...
for (code in keys(countryInfo))  {
  ...
  # x <- as.numeric(curCountryDf$DemocracyBad, na.rm=TRUE)
  # y <- as.numeric(curCountryDf$age, na.rm=TRUE)
  # if (is.na(x[1]) | is.na(y[1])) {
  #   cat("*****NULL Value - Country Name: ", countryInfo[[code]], "\n")
  #   rbind(resultDf, c(code, countryInfo[[code]], "NA", "NA"))
  #   next
  # }
  curCountryFit <- lm(
    formula = (x ~ y),
  ...
  summary(curCountryFit)
  coeff <- summary(curCountryFit)$coefficients[2,1]
  tstat <- summary(curCountryFit)$coefficients[2,3]
  # store the current result
  # cat(code, countryInfo[[code]], coeff, tstat, "\n")
  rbind(resultDf, c(code, countryInfo[[code]], coeff, tstat))
}
...
# use countryInfo(code, name) hash from Q4
# row: #countries, col: proportion of each rating
numRow <- length(unique(countryCode$country))
numCol <- 4
